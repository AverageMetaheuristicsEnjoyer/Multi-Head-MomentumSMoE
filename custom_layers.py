...
class GroupsFMoE(nn.Module):
    ...
    * `expert` can be specified as a module class, it is used to generate
    ...
    def forward(self, moe_inp):
        ...
        total_top_k = self.gate.num_groups * self.gate.top_k

        def structure_by_group_func(tensor):
            dim = tensor.shape[-1]
            tensor = tensor.view(-1, total_top_k, dim)
            tensor = tensor.view(-1, self.gate.num_groups, self.gate.top_k, dim)
            return tensor

        moe_outp = tree.map_structure(structure_by_group_func, fwd)

        gate_score = gate_score.view(-1, self.gate.num_groups, self.gate.top_k, 1)
